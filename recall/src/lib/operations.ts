...
export async function parseResume(rawText: string): Promise<ParsedResume> {
  const data = await callLLM(resumeParsePrompt(rawText), ResumeParseSchema, {
    maxTokens: 8192,
  });
...
export async function generateAnswer(args: {
  ...
  const needsRetry =
    tokensOf(short).length === 0 || spacingLooksBroken(short) || spacingLooksBroken(long);

  if (needsRetry) {
    const reasons = [
      tokensOf(short).length === 0 ? 'You must include 2 to 5 invented specifics.' : '',
