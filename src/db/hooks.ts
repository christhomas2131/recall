...
/** Persist a high-value mutation before its UI action is considered complete. */
export async function flushProjectNow(id: string): Promise<void> {
  const timer = timers.get(id);
  if (timer) clearTimeout(timer);
  timers.delete(id);
  await flushProject(id);
  while (inflight.has(id) || redo.has(id)) {
...
function readSettings(): Settings {
  try {
    ...
    const clean = { hasAcknowledged: stored.hasAcknowledged === true };
    // Purge browser-held provider credentials from pre-companion builds.
