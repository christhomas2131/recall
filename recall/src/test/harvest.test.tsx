...
function mockFurnace() {
  vi.stubGlobal('fetch', vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url.endsWith('/api/crawl/status')) return json(STATUS);
    if (url.endsWith('/api/crawl/shortlist')) return json({ rows: ROWS });
    if (url.endsWith('/api/crawl/start') && init?.method === 'POST') {
      started = true;
...
describe('Harvest against synthetic Furnace data', () => {
  ...
  it('explains itself when the Furnace is down instead of failing silently', async () => {
    online = false;
    render(<App />);
    expect(await screen.findByText(/Furnace not reachable/)).toBeInTheDocument();
    expect(screen.getByLabelText('Furnace address')).toHaveValue('http://localhost:8765');
    expect(screen.getByText(/furnace down/)).toBeInTheDocument();
  }, 30000);
});
