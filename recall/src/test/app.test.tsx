...
describe('the app runs', () => {
  ...
  it('routes to settings and stores the key in localStorage only', async () => {
    const user = userEvent.setup();
    render(<App />);
    await screen.findByRole('heading', { name: /writes wrong answers on purpose/i });
    await user.click(screen.getAllByRole('button', { name: /I understand/i })[0]);
    ...
    await waitFor(() => {
      expect(localStorage.getItem('recall.settings')).toContain('sk-test-123');
    });

    const provider = screen.getByRole('button', { name: 'OpenAI' });
    await user.click(provider);
    await waitFor(() => {
      expect(localStorage.getItem('recall.settings')).toContain('"provider":"openai"');
    });
  }, 30000);
});
...
describe('the export gate', () => {
  ...
  it('blocks in standard mode and permits in fast mode with a warning', async () => {
    const user = userEvent.setup();
    saveSettings({ apiKey: 'k', hasAcknowledged: true });
