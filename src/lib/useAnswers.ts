...
import { flushProjectNow, mutateQuestion } from '@/db/hooks';
...
export async function commitAnswer(
  ...
  await flushProjectNow(projectId);
}
  ...
  const generate = useCallback(
    async (question: Question, trigger: AnswerVersion['trigger'] = 'initial') => {
      ...
      try {
        ...
        await commitAnswer(project.id, question.id, answer, trigger);
      } catch (e) {
        if (isKeyProblem(e)) {
    ...
    async (target: Project, questionIds: string[]) => {
      ...
      await generateAnswersInBatch({
        ...
        onAnswer: (questionId, answer) => {
          void commitAnswer(target.id, questionId, answer, 'initial');
